...
local function filterSpell(e)
    if ((e.source.id == "disenchant" or e.source.id == "disenchant_dummy") and e.caster == tes3.player) then
        disenchantTry()
    end
end
...
local function triggerDisenchant(e)
    ...
    tes3.cast{ reference = tes3.player, spell = dummy, alwaysSucceeds = true}
    timer.start{type = timer.simulate, duration = 1, callback = function() tes3.deleteObject(dummy) end}
    end
end
